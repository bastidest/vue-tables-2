module.exports = function(h) {
  ...
  return (classes) => {
    ...
    data.map((row, index) => {
      ...
      let childRowView = (this.hasChildRow && this.openChildRows.includes(row[rowKey]))?(<tr class='VueTables__child-row'><td colspan={this.colspan}>{this._getChildRowTemplate(h, row)}</td></tr>):h();

      let transitionType = typeof this.opts.childRowTransition;
      if(transitionType === 'string') {
        rows.push(<transition name={this.opts.childRowTransition}>{childRowView}</transition>);
      } else if(transitionType && transitionType === 'object') {
        rows.push(h(this.opts.childRowTransition, {}, [
          childRowView
        ]));
      } else {
